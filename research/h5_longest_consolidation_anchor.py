"""H5: Anchor = peak before the longest consolidation.

Walking backward from sig:
  - Identify change-points: bars where high[A] > running_max(high[A+1..sig]).
  - Between consecutive change-points (cp_recent more recent, cp_distant older), measure the gap.
  - The LONGEST gap identifies the longest consolidation.
  - Anchor = cp_distant at the start (older end) of the longest gap.

Structurally: cp_distant defined the ceiling of the longest stretch where price never exceeded
that ceiling — that's the previous uptrend's peak before the longest consolidation.
"""
import pickle
import sqlite3
import numpy as np
...
def find_change_points(h, sig_idx):
    """Walk backward from sig_idx-1; collect bars where high exceeds running_max so far.
    Returns list of (A, high[A]) ordered from most-recent to most-distant."""
    cps = []
    running_max = h[sig_idx]
    for A in range(sig_idx - 1, -1, -1):
        if h[A] > running_max:
            cps.append((A, float(h[A])))
            running_max = h[A]
    return cps


def find_longest_consolidation_anchor(h, sig_idx):
    """Anchor = cp_distant at start of longest gap between consecutive change-points.
    cps[0] = most recent, cps[-1] = most distant.
    Gap between cps[i] and cps[i+1] measured forward in time = cps[i] - cps[i+1] bars.
    Also include the gap from sig_idx to cps[0] (the most recent stretch from sig_idx back to first cp)."""
    cps = find_change_points(h, sig_idx)
    if not cps:
        return None
    # Build gaps: each gap is (cp_recent_idx, cp_distant_idx, length)
    # Gap 0: from sig_idx to cps[0]: length = sig_idx - cps[0][0]
    # Gap i: between cps[i-1] and cps[i]: length = cps[i-1][0] - cps[i][0]
    gaps = []
    # First gap from sig to most-recent cp
    gaps.append((sig_idx, cps[0][0], sig_idx - cps[0][0], cps[0][0]))
    for i in range(1, len(cps)):
        cp_recent_A, _ = cps[i-1]
        cp_distant_A, _ = cps[i]
        length = cp_recent_A - cp_distant_A
        gaps.append((cp_recent_A, cp_distant_A, length, cp_distant_A))
    # Pick longest gap; tie-break by most recent (smallest cp_recent_A)
    gaps.sort(key=lambda g: (-g[2], g[0]))
    longest = gaps[0]
    return longest[3]  # cp_distant_A = anchor
...
for setup, ticker, entry_date in rows:
    ...
    total += 1
    A = find_longest_consolidation_anchor(h, sig_idx)
    if A is None:
        no_anchor += 1
        fail_details.append((setup, ticker, entry_date, "no change points"))
        continue
    ...
    records.append({
        ...
        "anchor_high": h[A], "sig_close": sig_close, "avwap": av,
        "passes": passes, "dates_arr": dates,
    })

print(f"=== T1.1 PASS RATE (rule = peak before longest consolidation) ===")
print(f"Total: {total}")

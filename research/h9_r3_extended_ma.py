"""H9: R3 with EXTENDED MA candidate set (SMA + EMA, periods 10/21/50/200).

Per Dan: for DRN, the 200 MA is right there as support — candles riding it. Lookback = 200.
For HTT, SMA200 was at 0.31 ADR but R3 was only checking SMA10/21/50/200 — actually it WAS in
my list but maybe not picked. Let me re-verify.

Includes BOTH SMA and EMA across periods 10/21/50/200. Pick longest MA within t=1.5 ADR.
Then R3: highest close in [sig - N_lso, sig - 1] where N_lso is that MA's period.
"""
import pickle
import sqlite3
import numpy as np
...
def lso_period_extended(c, sig_idx, adr, t_lso=1.5):
    """Longest period from {10, 21, 50, 200} where SOME MA (SMA or EMA) of that period is within t_lso ADR.
    Returns (period, ma_kind, ma_val)."""
    sig_close = c[sig_idx]
    qualifying = []
    for n in [10, 21, 50, 200]:
        if sig_idx < n - 1: continue
        sma_v = sma_at(c, n, sig_idx)
        ema_v = ema_at(c, n, sig_idx)
        for kind, v in [("SMA", sma_v), ("EMA", ema_v)]:
            if not np.isfinite(v): continue
            signed = (sig_close - v) / adr
            if abs(signed) <= t_lso:
                qualifying.append((n, kind, v, signed))
    if not qualifying:
        return min(10, max(2, sig_idx)), "fallback", float("nan")
    # Pick longest period; break ties by smaller signed (closer)
...
def r3_resistance(c, h, l, v, tp, sig_idx, adr):
    n, kind, ma_val = lso_period_extended(c, sig_idx, adr)
...
print(f"=== T1.1 PASS RATE (R3 with extended SMA+EMA) ===")
print(f"Total: {total}, Pass: {t1_pass}, Fail: {t1_fail}")
print()

print("=== ANCHOR vs DAN PICK on 8 failures ===")
print(f"{'ticker':<6} {'entry':<12} {'dan_anchor':<12} {'rule_anchor':<12} {'lso':>10} {'bars_off':>9} {'dan_avw':>8} {'rule_avw':>9} {'ratio':>6}")
bars_offs = []
ratios = []
for r in records:
    ...
    print(f"{r['ticker']:<6} {r['entry_date']:<12} {dan_anchor_date:<12} {r['anchor_date']:<12} {lso:>10} "
...
# Also: LSO MA used per setup (sanity check setup correlation)
print()
print("=== LSO MA period & kind by setup ===")
...
for setup in ["htf", "bf", "base"]:
    ...
    n = len(items)
    print(f"\n  {setup} (n={n}):")
    for ma, cnt in Counter(items).most_common(6):

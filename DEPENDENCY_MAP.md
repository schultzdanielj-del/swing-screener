...
- `local_runner/cache/expr_series/{TICKER}.append` — incremental append rows (raw float16 binary, one row per nightly append, wider than .npz: 15,805 + 196 = 16,001 cols)
...
| `local_runner/cache/expr_series/{TICKER}.state` | JSON (317 float64 values) | setup_forward_prop; overwritten nightly by forward-prop | expr_cache_builder (forward-prop) |

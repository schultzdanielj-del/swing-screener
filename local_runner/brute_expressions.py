...
def generate_dtss_lsp_expressions():
    """
    DTSS-specific LSP and AVWAP expressions.
    These require LSP context injected into ExpressionEngine.set_lsp_context().
    Only used in the DTSS example matrix and DTSS grinder — not universe matrix.
    """
    exprs = []

    # ═══════════════════════════════════════════════════════
    # LSP DISTANCE — How close is price to the structural high?
    # Positive = LSP above (approaching from below)
    # Negative = LSP below (breakout failure, IONQ-type)
    # ═══════════════════════════════════════════════════════
    for price_ref in ["C", "H"]:
        for norm in ["atr14", "adr14", "pct"]:
            exprs.append({
                "name": f"lsp_dist_{price_ref.lower()}_{norm}",
                "category": "dtss_lsp",
                "compute": {"op": "lsp_distance", "price_ref": price_ref, "normalizer": norm}
            })

    # ═══════════════════════════════════════════════════════
    # BOUNCE RECOVERY — How far has the rally retraced toward LSP?
    # 0.0 = still at post-LSP low, 1.0 = fully back at LSP
    # This captures "second rally into the same zone"
    # ═══════════════════════════════════════════════════════
    exprs.append({
        "name": "lsp_bounce_recovery",
        "category": "dtss_lsp",
        "compute": {"op": "lsp_bounce_recovery"}
    })

    # ═══════════════════════════════════════════════════════
    # RIGHT PEAK RATIO — Is the second top equal/lower/higher than LSP?
    # scan_high / lsp_price
    # 1.0 = exactly at LSP, >1.0 = breakout failure, <1.0 = lower high
    # ═══════════════════════════════════════════════════════
    exprs.append({
        "name": "lsp_right_peak_ratio",
        "category": "dtss_lsp",
        "compute": {"op": "lsp_right_peak_ratio"}
    })

    # ═══════════════════════════════════════════════════════
    # VOLUME CHARACTER — Is volume drying up on the second approach?
    # recent_avg_vol / lsp_bar_volume
    # Low = volume fading at resistance (bearish confirmation)
    # ═══════════════════════════════════════════════════════
    for avg_period in [3, 5, 10, 15, 20]:
        exprs.append({
            "name": f"lsp_vol_ratio_{avg_period}d",
            "category": "dtss_lsp",
            "compute": {"op": "lsp_volume_ratio", "avg_period": avg_period}
        })

    # ═══════════════════════════════════════════════════════
    # AVWAP FROM LSP BAR — Trapped buyer fuel
    # Price below AVWAP = average buyer from the LSP rally is underwater
    # = capitulation fuel for the short
    # ═══════════════════════════════════════════════════════
    for norm in ["atr14", "adr14", "pct"]:
        exprs.append({
            "name": f"avwap_lsp_{norm}",
            "category": "dtss_lsp",
            "compute": {"op": "avwap_lsp_distance", "normalizer": norm}
        })

    # ═══════════════════════════════════════════════════════
    # LSP STRUCTURAL METADATA
    # ═══════════════════════════════════════════════════════
    exprs.append({
        "name": "lsp_bars_back",
        "category": "dtss_lsp",
        "compute": {"op": "lsp_bars_back"}
    })
    exprs.append({
        "name": "lsp_prominence",
        "category": "dtss_lsp",
        "compute": {"op": "lsp_prominence"}
    })
    exprs.append({
        "name": "lsp_pullback_depth_atr",
        "category": "dtss_lsp",
        "compute": {"op": "lsp_pullback_depth"}
    })
...
def generate_dtss():
    """Full DTSS expression set: generic + bespoke LSP/AVWAP block."""
    return generate_all() + generate_dtss_lsp_expressions()

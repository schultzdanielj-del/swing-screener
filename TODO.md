...
### Infrastructure
5. **Remove Railway from nightly data flow** — ~~currently yfinance → Railway → local (round trip). Should be yfinance → local directly, Railway gets a backup copy. The nightly refresh should not depend on Railway for any compute or data. Railway is seed vault only.~~ **DONE (2026-03-28):** Task H Phase 3 Increment 1 rewired all OHLCV fetching to yfinance. `cache_builder.py` no longer calls Railway — uses `_yf_download()` directly for daily/weekly/monthly. HTF caches (weekly + monthly, 10yr lookback) merged into `cache_builder.py` (`--htf` to build, `--htf-status` to check). `nightly.py` step 1 replaced with local yfinance freshness check. `expr_cache_builder.py` uses HTF pickles instead of resampling, HTF skip logic removed. **Remaining:** Earnings step still calls Railway (broken endpoints). Need local Yahoo Finance scraper.

...
Step 10 of nightly refresh (7:00am ET, daily). Runs every morning regardless of whether there's new market data.

Backs up non-rebuildable data to Railway: SQLite tables (setups, examples, grind cycles, etc.) via REST API + grind result JSON files via file mirror. Non-rebuildable means human decisions and expensive compute results. Does NOT back up tables or files that can be cheaply regenerated by nightly pipeline or build scripts.

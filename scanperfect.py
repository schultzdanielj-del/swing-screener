...
class ScanTuningWorkspace(QFrame):
    ...
    def __init__(self, node_id="scan_tuning", parent=None):
        ...
        # Frequency stats (peak/avg per day/week/month/year)
    ...
    def _load_data(self):
        ...
        settings_path = cache_dir / ("scan_settings_%s.json" % setup) if cache_dir.exists() else None
        if settings_path and settings_path.exists():
            try:
                saved = json.loads(settings_path.read_text())
                e = saved.get("entry", {})
                self._setup_slider.setValue(e.get("setup_score_floor", 0))
                self._market_slider.setValue(e.get("market_score_floor", 0))
                if "refinement_depth" in e:
                    self._depth_slider.setValue(min(e["refinement_depth"], self._depth_slider.maximum()))
                self._wr_slider.setValue(int(e.get("wr_floor", 0) * 100))
                x = saved.get("exit", {})
                obj = x.get("objective", "sqn")
                for key, btn in self._obj_btns.items():
                    btn.setChecked(key == obj)
                self._update_obj_btn_styles()
                self._trim_slider.setValue(int(x.get("trim_pct", 0) * 100))
            except Exception as e:
                print(f"  ScanTuning: settings restore error: {e}")
    ...
    def _apply_filters(self):
        ...
        if n_total > 0:
            ...
            dates = [s.get("date", "") for s in surviving]
            date_counts = Counter(dates)
            peak_day = max(date_counts.values()) if date_counts else 0
            avg_day = n_total / len(date_counts) if date_counts else 0
            sorted_dates = sorted(d for d in dates if d)
            if len(sorted_dates) >= 2:
                try:
                    d0 = dt_date.fromisoformat(sorted_dates[0])
                    d1 = dt_date.fromisoformat(sorted_dates[-1])
                    span_days = (d1 - d0).days + 1
                except (ValueError, TypeError):
                    span_days = 1
            else:
                span_days = 1
            avg_week = n_total / max(span_days / 7, 1)
            avg_month = n_total / max(span_days / 30.44, 1)
            avg_year = n_total / max(span_days / 365.25, 1)
            self._freq_label.setText(
                "peak %d/day · avg %.1f/day\n"
                "%.1f/wk · %.1f/mo · %.0f/yr" % (
    ...
    def hideEvent(self, ev):
        """Auto-save settings when the workspace collapses."""
        super().hideEvent(ev)
        self._save_settings()
    ...
    def _save_settings(self):
        ...
        print("  Scan settings saved: %s" % path)
...
class SpyBubbleChart(QWidget):
    ...
    def paintEvent(self, ev):
        ...
        n_purple = sum(1 for b in self._bubbles if not b["is_winner"])
        p.setPen(QColor(C["text_dim"]))
        f.setPixelSize(10)
        p.setFont(f)
        p.drawText(QRectF(8, h - self.VOL_H - 16, 200, 14),
                   Qt.AlignLeft | Qt.AlignVCenter,
                   "%d signals (%d W / %d L)" % (len(self._bubbles), n_green, n_purple))

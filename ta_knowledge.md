...
- **ext50 carries the specific rulesets; ext200 is general TA.** The chop zone, upside_1/upside_2/downside_1/downside_2 reversal levels, and the cascading trendline structures emerged from observed 50-ext behavior and are battle-tested there. ext200 doesn't carry the same personally-validated rulesets — read it as general TA (200-SMA ceilings, per-stock 200-ext profile). The asymmetric ship of the 2026-04-25 cache build (Levels covers both ext50 and ext200; Trendlines is ext50-only) maps to this. Adding ext200 versions of ext50-specific expressions in future builds is harmless — the pipeline filters meaningless expressions downstream and only full rebuilds carry the small compute cost — but don't assume the ext200 outputs inherit the ext50 outputs' TA significance until they've earned it empirically.
...
- **Russian doll entries:** Nest the same AVWAP + MA confirmation framework at smaller timeframes to find the tightest possible entry. Once D1 daddy is confirmed, zoom in — stop only needs to account for the baby line confirmation. Elevated intraday RVOL (~2x+) on the entry candle confirms real participation. Risk defined by smallest timeframe, reward defined by the larger extension structure.

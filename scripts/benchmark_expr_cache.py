...
def run_optimized_ext_struct(engine, expressions, ext_indices, ext_name_to_idx, n_bars, data):
    """Run extension structure with vectorized linreg + cached bool_agg."""
    import json
    ...
    if not series_registry:
        ...
        results["ext_bool_agg"] = 0.0
        results["ext_other"] = 0.0
        return results

    # Classify ext indices
    linreg_indices = []
    bool_agg_indices = []
    other_indices = []
    ...
    for j in ext_indices:
        ...
        op = comp.get("op", "")
        if op == "on_series":
            inner_op_name = comp.get("inner_op", {}).get("op", "")
    ...
    # ── Linreg: vectorized stride tricks ──
    t0 = time.perf_counter()
    for j in linreg_indices:
        try:
            ...
            if series_name in series_registry:
                ...
                if comp["inner_op"]["op"] == "trendline_deviation":
            ...
            pass
    results["ext_linreg"] = time.perf_counter() - t0

    # ── Bool_agg: cache indicator booleans, numpy aggregation ──
    t0 = time.perf_counter()
    
    # Step 1: compute unique indicator booleans
    # Key = (series_name, json of bool_op spec)
    indicator_bool_cache = {}
    for j in bool_agg_indices:
        comp = expressions[j]["compute"]
        series_name = comp["series"]
        bool_spec = comp["bool_op"]
        cache_key = (series_name, json.dumps(bool_spec, sort_keys=True))
        
        if cache_key not in indicator_bool_cache:
            try:
                s_data = series_registry.get(series_name)
                if s_data is None:
                    indicator_bool_cache[cache_key] = None
                    continue
                # Compute inner indicator on extension series
                ...
                indicator_bool_cache[cache_key] = b.astype(bool)
            except:
                indicator_bool_cache[cache_key] = None

    # Step 2: pre-compute bars_since for since_true aggregations
    bars_since_cache = {}
    for j in bool_agg_indices:
        comp = expressions[j]["compute"]
        if comp["agg_op"] != "since_true":
            continue
        cache_key = (comp["series"], json.dumps(comp["bool_op"], sort_keys=True))
        if cache_key in bars_since_cache or indicator_bool_cache.get(cache_key) is None:
            continue
        b = indicator_bool_cache[cache_key]
        bs = np.full(n_bars, n_bars, dtype=np.float64)
        for i in range(n_bars):
            if b[i]:
                bs[i] = 0.0
                ...
                bs[i] = bs[i - 1] + 1.0
        bars_since_cache[cache_key] = bs

    # Step 3: dispatch all 760 bool_agg expressions with numpy
    for j in bool_agg_indices:
        comp = expressions[j]["compute"]
        cache_key = (comp["series"], json.dumps(comp["bool_op"], sort_keys=True))
        b = indicator_bool_cache.get(cache_key)
        if b is None:
            continue
        ...
        if agg_op == "count_true":
            data[:, j] = np_count_true(b, agg_period).astype(np.float32)
        elif agg_op == "since_true":
            bs = bars_since_cache.get(cache_key)
            if bs is not None:
                r = np.full(n_bars, np.nan)
                for i in range(agg_period - 1, n_bars):
                    r[i] = bs[i] if bs[i] < agg_period else -1.0
                data[:, j] = r.astype(np.float32)
        elif agg_op == "true_in_row":
            data[:, j] = np_true_in_row(b, agg_period).astype(np.float32)
    
    results["ext_bool_agg"] = time.perf_counter() - t0

    # ── Other on_series ops: use original compute_series ──
    t0 = time.perf_counter()
    for j in other_indices:
        try:
            ...
            pass
    results["ext_other"] = time.perf_counter() - t0
...
def main():
    print("\n" + "=" * 70)
    print("  EXPRESSION CACHE — TARGETED OPTIMIZATION BENCHMARK v5")
    print("=" * 70)
    ...
    # ═══ OPTIMIZED ═══
    print(f"\n  {'─' * 60}")
    print(f"  OPTIMIZED (numpy bools + vectorized linreg + HTF bools + ext bool_agg cache)")
    print(f"  {'─' * 60}")

...
def _validate_examples(examples, conditions, cache, expr_cache):
    ...
    for ex in examples:
        ...
        df = cache.get(ticker)
        if df is None:
            failures.append(f"{ticker}: not in OHLCV cache")
            continue
        ...
        if entry_date not in dates_str:
            failures.append(f"{ticker}: entry {entry_date} not in cache")
            continue
        ...
        if len(cached_dates) != len(df):
            failures.append(f"{ticker}: bar count mismatch")
            continue
        if scan_idx < 0 or scan_idx >= len(cached_data):
            failures.append(f"{ticker}: scan_idx out of range")
            continue
        row = cached_data[scan_idx, :]
        for cond in conditions:
            col_idx = name_to_idx.get(cond["name"])
            if col_idx is None:
                failures.append(f"{ticker}: {cond['name']} not in cache")
                break
...
def prune_conditions(conditions, cache, examples, expr_cache, workers, min_power):
    ...
    # Baseline
    print(f"  BASELINE: scanning with all {n_conds} conditions...")
    t = time.time()
    baseline = _parallel_scan(cache, cond_col_indices, cond_lows, cond_highs,
                               workers, expr_cache_dir)
    print(f"  Baseline: {baseline:,} passing bar-rows  ({time.time()-t:.0f}s)")
    ...
    for i, cond in enumerate(conditions):
        t_i = time.time()
        without = _parallel_scan(cache, cond_col_indices, cond_lows, cond_highs,
        ...
        cat = cond.get("category", "?")[:16]
        print(f"  [{i+1:2d}/{n_conds}] {flag}  power={power:+.3f}  "
              f"[{tier:>4}][{cat:>16}] {cond['name']}  ({time.time()-t_i:.0f}s)")

    # Required-condition check: if dropping it breaks any example, it stays
    required = set()
    if examples:
        for i, cond in enumerate(conditions):
            if powers[i] >= min_power:
                continue  # keeping anyway
            without = [c for j, c in enumerate(conditions) if j != i]
            fails = _validate_examples(examples, without, cache, expr_cache)
            if fails:
                required.add(cond["name"])
                print(f"  REQUIRED (example fails without): {cond['name']}")
    ...
    # Final validation on pruned set — hard requirement
    if examples:
        fails = _validate_examples(examples, kept, cache, expr_cache)
        if fails:
            print(f"\n  {'!'*60}")
...
# ══════════════════════════════════════════════════════════════
# SIGNAL SCANNER — identical computation path to signal_filter.py
# ══════════════════════════════════════════════════════════════

...
- Last-bar substitution: the bar's date comes from each quote's own `timestamp` (its ET session date), not the wall clock. Prior-session echoes are rejected first — a quote is skipped when its timestamp is from an earlier day or its `close` equals its own `previousClose`. (Right after the open the delayed feed briefly re-reports yesterday's completed daily bar; writing it would duplicate yesterday's candle onto today — the morning "doubled D1 candle" bug.) For a surviving quote: if its session date matches the cached last bar, O/H/L/C/V are overwritten in place (skipped if the close is unchanged); if it's newer, a new row is appended at that date. Tickers in the theme universe but missing from the main pickle (e.g., `BF.B` vs `BF-B`) are reported and skipped.

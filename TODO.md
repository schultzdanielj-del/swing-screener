...
| 9 | **Pyramidal grinder (Phase 1/2 replacement)** | Replace the current 2-phase system with a nested pyramid that progressively widens the historical window. Each tier locks in conditions from the previous tier and runs the spiderweb on the next time horizon. E.g.: today (11s) → 1 week (~10s) → 1 month (~30s) → 6 months (~30s) → 1 year (~30s) → 5 years (~30s). Each tier eliminates the cheap noise so the next tier only scores survivors. Total pipeline ~2 min instead of 28 min. Same spiderweb code, just different matrix construction per tier. Makes nightly re-grind across 20+ setups feasible. |

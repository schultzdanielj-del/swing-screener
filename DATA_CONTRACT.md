...
| `company_meta.json` | `fetch_company_meta.py` | Per-ticker identity data — Yahoo Finance `assetProfile` + `quoteType` for every ticker in `theme_map.UNIVERSE`. Shape: top-level `{built_at, n_tickers, n_success, n_errors, n_with_longName, n_with_longBusinessSummary, tickers: {TICKER: {longName, longBusinessSummary}}}`. Per-ticker entries with errors hold `{error: "..."}` instead of the two text fields. Resumable — re-runs skip tickers already cached unless `--retry-failed` is passed. UTF-8 encoded; reader must open with `encoding="utf-8"`. |
...
| `ext50_trendline_snapshots.json` | `ext50_trendline_snapshot_builder.py` | Per-UNIVERSE-ticker u1/u2/u3 + l1/l2/l3 50-SMA-extension trendline equations at the most recent EOD bar (partial intraday bar dropped). Top-level fields: `built_at`, `built_in_seconds`, `source_cache`, `intraday_partial_dropped` (bool), `n_universe`, `n_snapshots`, `n_errors`, `tickers` (dict). Per-ticker payload: `asof_bar`, `asof_date`, `asof_ext50`, `u: [{i0, v0, i1, v1, slope, anchor_type, proj_asof, signed_dist, span}, ...]` (0-3 entries, descending lines sorted by ascending `abs(signed_dist)`), `l: [...]` (ascending lines, same schema). Sign convention: `signed_dist = proj_asof − ext_asof` (positive = price below line). Lines that fail the strict full-line break check (`scripts/ext50_trendlines._has_line_break`) are excluded from `u`. Consumed only by `theme_dashboard.py`'s Setups page; see `EXT50_TRENDLINE_SNAPSHOTS.md`. UTF-8 encoded JSON. |

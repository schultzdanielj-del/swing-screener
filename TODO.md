...
3b. **Consensus pipeline speed optimizations** — Signal grinds are the bottleneck (~10 hrs sequential for 30 runs). Options to reduce further beyond parallelism:
    - ✅ **Parallel batch execution (--parallel N)** — DONE (2026-03-26). Launches N grinds simultaneously via Popen. Default 4. 30 runs ÷ 4 = 8 batches. Estimated ~2.5 hrs instead of 10. `--smoke-test` flag for quick 2-min verification (4 grinds in one batch).
    - **Reduce run count (10+10 instead of 15+15)** — Meinshausen recommends 10-20 for expensive searches. 10+10 = 20 runs instead of 30. Slightly wider confidence intervals but 33% less time. Combined with parallel=4: ~1.5 hrs.
    - **Shared memory OHLCV cache** — Every subprocess loads the same ~2GB pickle independently. `multiprocessing.shared_memory` or mmap loads it once, all processes read the same physical memory. Saves ~15s startup per run × 30 = ~7 min. Minor win but cleaner memory footprint.
    - **Shallow permuted runs** — Permuted runs only establish the noise floor (how many conditions noise produces). The first 2-3 tiers (D1, 1wk, 1mo) likely capture enough of the noise floor without running 6mo/1yr/5yr. Could halve permuted run time. Risk: if noise conditions cluster in deeper tiers, the null distribution would be too tight.
    - **Per-run timing: unknown until first parallel smoke test.** The smoke test prints a "Full run estimate" extrapolation. Actual timings will determine which additional optimizations are worth pursuing.

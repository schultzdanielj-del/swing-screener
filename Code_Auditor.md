...
A manual code review tool. When you're ready to check your work, run `./audit.sh` in Git Bash. It uses Claude Code in pipe mode (`claude -p`) to read the diff, the relevant spec docs, the dependency map, and downstream consumer files, then evaluates four criteria:
...
That's it. No flags, no arguments. It figures out the diff range automatically.
...
### If it FAILs
The auditor does NOT advance its bookmark on FAIL. Fix the issues, commit, and run `./audit.sh` again. It will re-audit from the same starting point, catching both the original problems and your fixes.

### Force re-audit
Delete the state file to start fresh:
```
rm local_runner/cache/last_audited_commit.txt
./audit.sh
```
...
═══════════════════════════════════════════════════════════
  ...
  Fix the issues and re-run ./audit.sh
```
...
**Spec mapping:** The grep block in `audit.sh` maps changed filenames to spec docs. When you add a new component or spec doc, add a line to the mapping.
